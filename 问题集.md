...
另外一个想法. Lagrange 中值定理中的表达式 $\frac{f(b)-f(a)}{b-a}$ 可以视作导数的近似: 当 $a,b$ 足够接近时, 这个表达式接近 $a$ 处的导数, 而 Lagrange 中值定理说的是, 它恰等于 $a,b$ 中间某处的导数.
以此类推, 是否可以由一个映射 $\mathbb{R}^n\to \mathbb{R}^m$ 在球面 $S^{n-1}$ 上的取值写出微分的近似, 使得当球面半径足够小时这个表达式接近某处的微分, 而一般情形下它恰等于球体内部某处的微分?
